...
public class GameSizingComputer {
    ...
    public net.gazeplay.games.bera.GameSizing computeGameSizing(Scene scene) {
        Rectangle2D bounds = new Rectangle2D(0, 0, scene.getWidth(), scene.getHeight());

        return computeGameSizing(bounds);
    }

    public net.gazeplay.games.bera.GameSizing computeGameSizing(Pane pane) {
        Rectangle2D bounds = new Rectangle2D(0, 0, pane.getWidth(), pane.getHeight());

        return computeGameSizing(bounds);
    }

    public net.gazeplay.games.bera.GameSizing computeGameSizing(Dimension2D dimension2D) {
        Rectangle2D bounds = new Rectangle2D(0, 0, dimension2D.getWidth(), dimension2D.getHeight() * 0.9);

        return computeGameSizing(bounds);
    }

    public net.gazeplay.games.bera.GameSizing computeGameSizing(Rectangle2D bounds) {
        ...
        net.gazeplay.games.bera.GameSizing gameSizing = new net.gazeplay.games.bera.GameSizing(width / nbColumns, height / nbLines, shift);
